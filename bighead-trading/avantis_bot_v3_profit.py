...
class Config:
    STRATEGY_NAME = "Strategy V3 - PROFIT OPTIMIZED"
    STRATEGY_VERSION = "3.0.0 - SOL Focus + Bigger Wins"
    ...
    # Capital Allocation - V3 PROFIT OPTIMIZED
    TOTAL_CAPITAL = 60.0
    ASSETS = {
        'SOL': {'capital': 40.0, 'pair_index': 9},   # 65% on best performer (+$2.96/trade in backtest)
        'ARB': {'capital': 20.0, 'pair_index': 4},   # 35% on proven secondary
        'OP': {'capital': 0.0, 'pair_index': 7},     # Disabled (low performance)
        'ETH': {'capital': 0.0, 'pair_index': 0}     # Disabled (unprofitable)
    }
    
    # Strategy Parameters
    TIMEFRAME = '15m'
    LEVERAGE = {
        'SOL': 10,  # SOL max is 10x (like OP)
        'ARB': 15,  # ARB supports 15x
        'OP': 10,   # OP max is 10x
        'ETH': 15   # ETH supports high leverage
    }
    RISK_PER_TRADE = 0.03  # 3%
    RR_RATIO = 2.5  # V3: Increased from 2.0 for 25% bigger wins
    MIN_POSITION_SIZE = 12.0  # Avantis minimum collateral requirement (with safety buffer)
    
    # Position Limits
    MAX_POSITIONS_PER_ASSET = 2  # Reduced for SOL/ARB only
    ...
    BREAKEVEN_AT = 0.4    # V3: Earlier breakeven (40% vs 50%)
    TAKE_PARTIAL_AT = 0.5  # Take 50% profit at 50% to TP
    PARTIAL_SIZE = 0.5     # Close 50% of position
    
    # Trailing Stop Loss
    USE_TRAILING_SL = True
    TRAILING_SL_ACTIVATION = 0.08  # V3: Easier to hit (8% vs 10%)
    TRAILING_SL_DISTANCE = 0.005   # Trail 0.5% below highest price
    
    # Filters
    USE_VOLUME_FILTER = True
    VOLUME_THRESHOLD = 2.0  # V3: Higher quality trades (2.0x vs 1.5x)
    ...
    # Logging
    LOG_FILE = 'strategy_v3_profit.log'
    TRADE_LOG = 'strategy_v3_profit_trades.csv'
    ...
    async def run(self):
        ...
        try:
            while self.running:
                ...
                # Fetch prices
                prices = {}
                # Only check active assets (capital > 0)
                active_assets = [asset for asset, config in Config.ASSETS.items() if config['capital'] > 0]
                ...
                # Check for new signals (active assets only)
                for asset in active_assets:
