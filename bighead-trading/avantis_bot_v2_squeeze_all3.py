...
class Config:
    STRATEGY_NAME = "Strategy 1 V2 Squeeze + All 3"
    STRATEGY_VERSION = "2.2.0 - Ultimate"
    ...
    BAD_HOURS_UTC = [0, 1, 2, 3, 4, 5]  # Avoid midnight-6am UTC
    BEST_HOURS_UTC = [13, 14, 15, 16, 17, 18]  # US session peak
    ...
    # Logging
    LOG_FILE = 'strategy1_v2_squeeze_all3.log'
    TRADE_LOG = 'strategy1_v2_squeeze_all3_trades.csv'
...
class SMCIndicators:
    ...
    def add_indicators(df, swing_length=3, lookback=20):
        ...
        if Config.USE_ATR_SL:
            df['atr'] = TechnicalIndicators.atr(df, Config.ATR_PERIOD)
        ...
        if self.direction == 'LONG':
            ...
            if self.trailing_active:
                ...
                if new_sl > self.sl:
                    old_sl = self.sl
                    self.sl = new_sl
                    logger.trade(f"📈 Trailing SL: {self.asset} ${old_sl:.4f} → ${new_sl:.4f}")
                    return True
...
class PositionManager:
    ...
    def check_risk_limits(self):
        ...
        if self.consecutive_losses >= Config.CONSECUTIVE_LOSS_LIMIT:
            logger.error(f"⛔ CONSECUTIVE LOSS LIMIT: {self.consecutive_losses} in a row")
...
# ============================================================================
# TRADING ENGINE WITH ALL FILTERS
# ============================================================================
...
class TradingEngine:
    ...
    def check_time_filter(self):
        ...
        if current_hour in Config.BAD_HOURS_UTC:
            logger.info(f"   Skipped: Bad trading hour ({current_hour}:00 UTC)")
            return False
    ...
    def check_filters(self, asset, df, signal):
        ...
        if Config.USE_TREND_FILTER:
            if signal == 1 and not latest['trend_bullish']:
                logger.info(f"   Skipped {asset}: LONG but trend bearish")
                return False
            if signal == -1 and latest['trend_bullish']:
                logger.info(f"   Skipped {asset}: SHORT but trend bullish")
                return False
        
        # NEW: RSI Filter
        if Config.USE_RSI_FILTER:
            rsi = latest['rsi']
            if signal == 1 and rsi > Config.RSI_OVERBOUGHT:
                logger.info(f"   Skipped {asset}: LONG but RSI overbought ({rsi:.1f})")
                return False
            if signal == -1 and rsi < Config.RSI_OVERSOLD:
                logger.info(f"   Skipped {asset}: SHORT but RSI oversold ({rsi:.1f})")
                return False
        
        # Squeeze filter
        if Config.USE_SQUEEZE_FILTER:
            if not latest['sqz_off']:
                logger.info(f"   Skipped {asset}: Squeeze not OFF")
                return False
        ...
        filter_summary = []
        if Config.USE_VOLUME_FILTER:
            filter_summary.append(f"Vol:{latest['volume_ratio']:.1f}x")
        if Config.USE_RSI_FILTER:
            filter_summary.append(f"RSI:{latest['rsi']:.0f}")
        if Config.USE_SQUEEZE_FILTER:
            filter_summary.append(f"SqzMom:{latest['sqz_mom']:.3f}")
        
        logger.info(f"   ✅ All filters PASSED: {asset} ({', '.join(filter_summary)})")
        return True
    
    def calculate_position_size(self, asset, entry, sl, atr=None):
        """Calculate position size with optional ATR-based SL"""
    ...
    def calculate_sl_tp(self, df, entry, direction):
        """Calculate SL and TP with ATR if enabled"""
        latest = df.iloc[-1]
        
        # NEW: ATR-based SL
        if Config.USE_ATR_SL and 'atr' in df.columns:
            atr = latest['atr']
            
            if direction == 'LONG':
                sl = entry - (atr * Config.ATR_MULTIPLIER)
                tp = entry + (atr * Config.ATR_MULTIPLIER * Config.RR_RATIO)
            else:
                sl = entry + (atr * Config.ATR_MULTIPLIER)
                tp = entry - (atr * Config.ATR_MULTIPLIER * Config.RR_RATIO)
            
            logger.info(f"   Using ATR SL: ATR=${atr:.4f}, SL dist={abs(entry-sl)/entry*100:.2f}%")
        
        # Fallback: Range-based or fixed
        else:
            if direction == 'LONG':
                sl = latest['range_low'] if latest['range_low'] < entry else entry * 0.985
                tp = entry + (entry - sl) * Config.RR_RATIO
            else:
                sl = latest['range_high'] if latest['range_high'] > entry else entry * 1.015
                tp = entry - (sl - entry) * Config.RR_RATIO
        
        return sl, tp
    
    async def check_signals(self, asset):
        # NEW: Check time filter first
        ...
        sl, tp = self.calculate_sl_tp(df, current_price, direction)
        
        # Calculate size
        atr = latest.get('atr', None)
        size = self.calculate_position_size(asset, current_price, sl, atr)
        if size is None:
            return
        ...
        filter_info = []
        if Config.USE_VOLUME_FILTER:
            filter_info.append(f"Vol: {latest['volume_ratio']:.2f}x")
        if Config.USE_RSI_FILTER:
            filter_info.append(f"RSI: {latest['rsi']:.0f}")
        if Config.USE_SQUEEZE_FILTER:
            filter_info.append(f"Sqz: OFF, Mom: {latest['sqz_mom']:.3f}")
        if Config.USE_ATR_SL:
            filter_info.append(f"ATR: ${atr:.4f}")
    ...
    async def run(self):
        ...
        logger.info(f"Version: {Config.STRATEGY_VERSION}")
        logger.info("="*70)
        logger.info(f"Enhancements (15 total):")
        ...
        logger.info(f"  ✅ Trailing SL: {Config.TRAILING_SL_ACTIVATION*100}% activation, {Config.TRAILING_SL_DISTANCE*100}% trail")
        ...
        logger.info(f"  ✅ Trend alignment: 20 EMA")
        logger.info(f"  ✅ Squeeze filter: {'ON' if Config.USE_SQUEEZE_FILTER else 'OFF'}")
        logger.info(f"  ✅ ATR-based SL: {'ON' if Config.USE_ATR_SL else 'OFF'} ({Config.ATR_MULTIPLIER}x ATR)")
        logger.info(f"  ✅ Time filter: {'ON' if Config.USE_TIME_FILTER else 'OFF'} (avoid hours {Config.BAD_HOURS_UTC})")
        logger.info(f"  ✅ RSI filter: {'ON' if Config.USE_RSI_FILTER else 'OFF'} (OB:{Config.RSI_OVERBOUGHT}/OS:{Config.RSI_OVERSOLD})")
        logger.info(f"  ✅ Consecutive loss protection: {Config.CONSECUTIVE_LOSS_LIMIT} limit")
        logger.info(f"  ✅ Dynamic risk adjustment")
        logger.info(f"  ✅ Enhanced logging")
        logger.info(f"  ✅ Discord notifications")
        logger.info("="*70)
        
        await send_discord_notification(
            f"🚀 **{Config.STRATEGY_NAME} Started**\n"
            f"✅ 15 total enhancements active\n"
            f"✅ ATR SL + Time Filter + RSI Filter\n"
        ...
        try:
            while self.running:
                ...
                if self.position_manager.positions and prices:
                    logger.info(f"{'#':<4} {'Asset':<6} {'Side':<6} {'Entry':<12} {'SL':<12} {'TP':<12} {'Unrealized':<12} {'Realized':<12} {'Flags':<20}")
                    logger.info("-"*110)
                    ...
                    for idx, pos in enumerate(self.position_manager.positions, 1):
                        if pos.asset in prices:
                            ...
                            logger.info(
                                ...
                                f"${position_pnl:+11.2f} ${'0.00':<11} {flags_str:<20}"
                            )
